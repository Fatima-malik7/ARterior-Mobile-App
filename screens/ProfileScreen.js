...
import { View, Text, Image, TouchableOpacity, ScrollView, Switch, Pressable, Alert } from 'react-native';
          ...
          onPress: async () => {
            try {
              ...
              if (response.ok) {
                ...
                console.error('Delete failed:', data.message || response.statusText);
      ...
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 100, flexGrow: 1 }} keyboardShouldPersistTaps="handled">
        ...
        {user && (
          <>
            <View style={[styles.section, { backgroundColor: colors.card }]}>
              <ProfileItem icon="person-outline" label={i18n.t('editProfile')} onPress={() => navigation.navigate('EditProfile')} />
              <ProfileItem icon="language-outline" label={i18n.t('language')} value={language === 'en' ? 'English' : 'اردو'} onPress={() => changeLanguage(language === 'en' ? 'ur' : 'en')} />
            </View>

            <View style={[styles.section, { backgroundColor: colors.card }]}>
              <ProfileItem icon="lock-closed-outline" label={i18n.t('changePassword')} onPress={() => navigation.navigate('ChangePassword')} />
              <ProfileItem icon="color-palette-outline" label={i18n.t('theme')}>
                <Switch value={isDarkTheme} style={styles.value2} onValueChange={toggleTheme} />
              </ProfileItem>
            </View>

            <View style={[styles.section, { backgroundColor: colors.card }]}>
              <ProfileItem icon="help-circle-outline" label={i18n.t('helpSupport')} onPress={() => navigation.navigate('HelpSupport')} />
              <ProfileItem icon="chatbox-ellipses-outline" label={i18n.t('contactUs')} onPress={() => navigation.navigate('ContactUs')} />
              <ProfileItem icon="document-text-outline" label={i18n.t('privacyPolicy')} onPress={() => navigation.navigate('PrivacyPolicy')} />
            </View>

            <View style={[styles.section, { backgroundColor: colors.card }]}>
              <ProfileItem icon="trash-outline" label={i18n.t('deleteAccount')} onPress={handleDeleteAccount} />
            </View>
          </>
        )}
        ...
        {!user && (
          <TouchableOpacity onPress={() => navigation.navigate('Login')} style={styles.logoutButton}>
            <Text style={styles.logoutText}>{i18n.t('loginSignup')}</Text>
